...
if [ "$1" == "server" ]; then
    ...
    ssh -t $host "export MEDIA_MONITOR_ORF_API_PASSWORD=$MEDIA_MONITOR_ORF_API_PASSWORD && export MEDIA_MONITOR_OPENAI_KEY=$MEDIA_MONITOR_OPENAI_KEY && export MEDIA_MONITOR_YOUTUBE_KEY=$MEDIA_MONITOR_YOUTUBE_KEY && cd $host_dir && ./docker/control.sh stop && ./docker/control.sh start && ./docker/control.sh logs"
else
